  ...
  return (
    <main className="min-h-screen bg-white text-black">
      ...
      >
        ...
        {/* NEW: Customize section (Coming soon) */}
        <div className="mt-10">
          <div
            className="rounded-3xl border p-6 shadow-sm flex items-center justify-between flex-col sm:flex-row gap-4"
            style={{
              borderColor: "#e5e7eb",
              backgroundColor: "#fff",
              color: "#000",
          ...
          >
            <div>
              <h3 className="text-lg font-semibold">Customize your bundle</h3>
              <p className="mt-1 text-sm" style={{ color: "#6b7280" }}>
                AI-powered personalization to tailor plans, services, and
                add-ons to your exact needs.
              </p>
            </div>
            <button
              type="button"
              disabled
              className="rounded-xl px-4 py-2 font-semibold opacity-60 cursor-not-allowed"
            ...
            return (
              <li key={s.title}>
                <button
                  type="button"
                  onClick={() => {
                    const isActive = selectedServices.has(s.title);
                    setSelectedServices((prev) => {
                      const next = new Set(prev);
                      isActive ? next.delete(s.title) : next.add(s.title);
                      return next;
                    });
                  }}
                  aria-pressed={active}
      ...
      <section id="contact" className="relative mx-auto max-w-7xl px-6 pb-24">
        ...
        >
          <div className="grid items-center gap-10 p-8 sm:p-12 lg:grid-cols-2">
            ...
            {/* Hide the image on mobile; show from lg+ */}
            <div className="relative h-64 w-full lg:h-80 hidden lg:block">
              <Image
